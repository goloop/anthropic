...
// Wire names of the capabilities a request can ask for. This provider has no
// response_format of its own - a Format is asked for in the system prompt, so
// the provider can never reject one - which is why the format list is empty.
var (
...
func (c *Client) Capabilities() ai.Capabilities {
	// Everything this driver offers for structured output is asked for in the
	// system prompt, so every mode is emulated - including Strict, which this
	// provider has no way to enforce.
	emulated := ai.FormatCapability{
		JSON:       ai.FormatEmulated,
		...
		Hosted: map[ai.HostedKind]ai.HostedCapability{
			ai.HostedWebSearch: {
				Web: &ai.HostedWebCapability{
					...
					AllowDomains: true,
					BlockDomains: true,
					// The provider takes one list or the other, never both.
					BothDomainLists: false,
					Region:          true,
				},
				Policy: ai.HostedPolicyCapability{Required: true},
				// A search and a format hold together here precisely because
				// the format is emulated: there is no native constraint on the
				// output for the search to conflict with.
				WithFormat: emulated,
			},
		},
	}
